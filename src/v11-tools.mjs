...
export function recommendedContacts(buyerType = "") {
...
export function riskBasedActions(lead, authenticity) {
  const actions = [];
...
function channelFromStage(stage, lead) {
...
export function buildEnhancedDevelopmentPlan(lead, authenticity, product) {
  const stage = stageFromScore(authenticity.score);
  const contacts = recommendedContacts(lead.buyerType);
  const channel = channelFromStage(stage, lead);
