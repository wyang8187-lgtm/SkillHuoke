...
```text
v1.3 - 后端搜索服务、服务端 API Key、官网 Contact 页面解析
```
...
- 搜索失败时提示后续 V1.3 后端服务处理跨域和 API Key 安全问题
- 新增本地后端 `/api/search`，API Key 放在 `.env`，不再暴露在前端页面
- 后端会尝试解析官网首页、Contact、About 页面，提取公开邮箱、电话、WhatsApp 和 LinkedIn
...
如果你希望使用联网搜索和官网 Contact 页面解析，先在项目根目录创建 `.env`：

```text
GOOGLE_SEARCH_API_KEY=你的 Google Custom Search API Key
GOOGLE_SEARCH_CX=你的 Google CX
BING_SEARCH_API_KEY=你的 Bing Search API Key
...
当前版本是本地 MVP。V1.3 已经支持本地后端联网搜索和公开 Contact 页面解析，但还不是完整的云端自动化获客系统：

- GitHub Pages 静态演示不包含后端搜索，需要本地运行 `npm start`
- 搜索 API Key 需要你自己配置到 `.env`
